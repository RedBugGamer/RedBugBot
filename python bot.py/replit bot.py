...
async def on_message(message):
    ...
    if message.author == client.user:
        ...
        status=True
        running = True
        await message.reply(embed=getstatuscolor(exa.get_server(id=id).status,datetime.now()))
        
    elif message.content == "T!web":
        await message.reply(embed=discord.Embed(description="Meine [Website](https://Website-RedBugBot.redbuggamer.repl.co)"))
    ...
    elif message.content.startswith("T!suggest"):
        suggestion = message.content.replace("T!suggest","")
        await message.channel.send(embed=discord.Embed(description=f"Du hast `{suggestion}` suggestet"))
        TinyDB("suggestion.json").insert(Mapping("suggestion",suggestion))
