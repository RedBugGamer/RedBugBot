...
async def garbagecollection():
    ...
    garbagenum += len(
        cursor.execute(
            "SELECT * FROM errors WHERE expires < ?", (datetime.datetime.now(),)
        ).fetchall()
    )
    ...
    cursor.execute("delete from embeds where expires < ?",
                   (datetime.datetime.now(),))
    cursor.execute("delete from errors where expires < ?",
                   (datetime.datetime.now(),))
    connection.commit()
    ...
    try:
        ...
        if message.content == "T!reboot":
            if message.author.id == redbuggamer:
                ...
                await client.change_presence(
                    status=nextcord.Status.dnd, activity=nextcord.Game(
                        "Rebooting")
                )
                if githubcooldown != 0:
        ...
        if not message.author.id in blockedusers:
            ...
            if message.content == "T!help":
                ...
                Help.set_author(name=client.user,
                ...
                if message.author.id == redbuggamer:
                    ...
                    if message.content.replace("T!bind ", "") == "unbind":
                        ...
                        if (
                            len(
                                cursor.execute(
                                    "SELECT * FROM exaroton where serverid = ?", (
                                        customid,)
                                ).fetchall()
                            )
                            == 0
                ...
                if "error" in request:
                    ...
                    level = request["level"]
                    levelbarpercent = math.ceil(
                        (math.floor(level) - level) * -12)
                    levelbar = ""
                    ...
                    playerembed.add_field(
                        inline=False, name="`uuid`", value=uuid)
                    playerembed.add_field(
                ...
                if message.author.id == redbuggamer:
                    match message.content.split()[1]:
                        ...
                        case "error":
                            try:
                                id, msg, errormsg, user, expires = cursor.execute(
                                    "SELECT * FROM errors where id = ?", (int(message.content.split()[2]),)).fetchall()[0]
                                expires: datetime.datetime
                                # + datetime.timedelta(days=-30)
                                await message.reply(embed=nextcord.Embed().add_field(name="Id", value=id).add_field(name="Message", value=msg).add_field(name="Error", value=errormsg).add_field(name="User", value=user).add_field(name="Expires", value=str(expires)))
                                cursor.execute(
                                    "DELETE from errors where id =?", (id,))
                                connection.commit()
                            except IndexError:
                                await message.reply(embed=nextcord.Embed(color=nextcord.Color.red(), description="Die id existiert nicht"))
                        case "raise":
        ...
        a = cursor.execute("SELECT * FROM errors").fetchall()

        id_ = list(set(range(0, len(a) + 1)).difference(i[0] for i in a))[0]
        ...
        await message.reply(embed=nextcord.Embed(color=nextcord.Color.red(), title="An unknown Error occurred", description="An unknown Error has occured. If this Error occurs again please message RedBugGamer#2069 with the Error id `"+str(id_)+"`."))
        await log_in_channel("Error id " + str(id_), nextcord.Color.red())
